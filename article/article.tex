...
%             относительно классической нейронной сети возрастает в 2 раза.
    ...
    Предсказательное распределение (\ref{pred_distribution})
    ...
    с применением методов Монте--Карло для марковских цепей (MCMC).
    Однако эти методы требуют большого количества вычислительных ресурсов,
    что делает их неприменимыми для больших датасетов и большого числа весов.
    ...
    $q_{\pmb{\theta}}(\pmb{W})$,
    $T$ и $S$~--- количество сэмлов для аппроксимации,
    ...
    Также при градиентной оптимизации компоненты вектора $\pmb{\sigma}$
    ...
    со знаком минус и,
    разделив на размер обучеющей выборки L,
    ...
    классическая (без регуляризации) и байесовская.
