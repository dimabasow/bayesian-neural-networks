...
class _BayesianNormBase(BayesianModule):
    ...
    def __init__(
        ...
        if self.affine:
            ...
            self.get_weight_and_bias = self.get_weight_and_bias_not_affine
        if self.track_running_stats:
            ...
            self.register_buffer(
                ...
                torch.tensor(
                    ...
                    dtype=torch.long,
                    **{
                        k: v for k, v in factory_kwargs.items()
                        if k != 'dtype'
                    }
                )
            )
    ...
    def get_weight_and_bias_affine(self) -> tuple[torch.Tensor, torch.Tensor]:
        return (self.weight() + 1, self.bias())

    def get_weight_and_bias_not_affine(self) -> tuple[None, None]:
        return None, None


class _BayesianBatchNorm(_BayesianNormBase):
    def __init__(
        self,
        num_features: int,
        eps: float = 1e-5,
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        """
        weight, bias = self.get_weight_and_bias()
...
class BayesianBatchNorm1d(_BayesianBatchNorm):
    def _check_input_dim(self, input):
...
class BayesianBatchNorm2d(_BayesianBatchNorm):
    def _check_input_dim(self, input):
        if input.dim() != 4:
            raise ValueError(f"expected 4D input (got {input.dim()}D input)")


class BayesianBatchNorm3d(_BayesianBatchNorm):
    def _check_input_dim(self, input):
        if input.dim() != 5:
