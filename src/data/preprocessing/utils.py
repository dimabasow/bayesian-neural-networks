...
def drop_columns_empty(df: pl.DataFrame) -> pl.DataFrame:
    columns_to_drop = []
...
def drop_columns_empty_or_constant(df: pl.DataFrame) -> pl.DataFrame:
    columns_to_drop = []
    for column in df.columns:
        series = df[column]
        series_drop_null = series.drop_nulls().drop_nans()
...
def drop_columns_constant(df: pl.DataFrame) -> pl.DataFrame:
    columns_to_drop = []
    for column in df.columns:
        series = df[column]
        series_drop_null = series.drop_nulls().drop_nans()
